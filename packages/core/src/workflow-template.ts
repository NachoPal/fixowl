...
export interface WorkflowTemplateOptions {
  ...
  /**
   * Pre-work triage toggles (all default ON). The action defaults them true, so
   * each input is rendered only when a repo opts OUT (value false); a default-on
   * repo's workflow is byte-for-byte as before. See docs/issue-triage.md.
   */
  skipAlreadyFixed?: boolean;
  skipDuplicates?: boolean;
  verifyBeforeFix?: boolean;
  /**
   * Opt-in priority-label selection (see docs/priority-selection.md). Rendered
   * only when `labels` is non-empty, so a repo that does not use it keeps a
   * byte-for-byte workflow and the action's own empty default applies.
   */
  priority?: { labels: string[]; includeUnlabeled: boolean };
  /**
   * Opt-in bounded concurrency (issue #36); the input is rendered only when it
   * differs from the default (1), so a repo not using it keeps a byte-for-byte
   * workflow and the action's own "1" default applies. See docs/parallel-execution.md.
   */
  maxParallel?: number;
...
${dispatchBlock}`;
  ...
  // Triage toggles default ON in the action; render an input only on opt-out so a
  // default-on workflow is byte-for-byte as before (see docs/issue-triage.md).
  if (options.skipAlreadyFixed === false) {
    withLines.push(`          skip-already-fixed: "false"`);
  }
  if (options.skipDuplicates === false) {
    withLines.push(`          skip-duplicates: "false"`);
  }
  if (options.verifyBeforeFix === false) {
    withLines.push(`          verify-before-fix: "false"`);
  }
  // Priority selection is opt-in; render the inputs only when labels are set, so
  // a repo not using it keeps a byte-for-byte workflow. include-unlabeled defaults
  // ON in the action, so render it only on opt-out.
  if (options.priority !== undefined && options.priority.labels.length > 0) {
    withLines.push(`          priority-labels: "${options.priority.labels.join(",")}"`);
    if (options.priority.includeUnlabeled === false) {
      withLines.push(`          priority-include-unlabeled: "false"`);
    }
  }
  // Bounded concurrency is opt-in; render the input only when it differs from
  // the default (1), so a repo not using it keeps a byte-for-byte workflow.
  if (options.maxParallel !== undefined && options.maxParallel !== 1) {
    withLines.push(`          max-parallel: "${options.maxParallel}"`);
  }
...
jobs:
  fixowl:
    ...
    steps:
      ...
      # Stale lane worktrees (issue #36's bounded concurrency, "$GITHUB_WORKSPACE/worktrees")
      # from a killed run are cleared too, so a re-run never reuses a half-torn-down lane.
      - name: Reset workspace git state
        run: rm -rf "$GITHUB_WORKSPACE/.git" "$GITHUB_WORKSPACE.fixowl-git" "$GITHUB_WORKSPACE/worktrees"
