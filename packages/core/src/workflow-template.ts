...
export function renderFixowlWorkflow(options: WorkflowTemplateOptions): string {
  ...
  // The `source` input lets the local host scheduler tag its dispatch
  // (source: host-scheduler), which the run-name surfaces as a marker and
  // the action's once-a-day budget guard reads. Manual runs leave it blank.
  const dispatchBlock = `  workflow_dispatch:
    inputs:
      source:
        description: "Internal: the local host scheduler sets this to host-scheduler; leave blank for manual runs."
        required: false
        default: ""`;
...
name: fixowl
run-name: "fixowl night run\${{ github.event.inputs.source == 'host-scheduler' && ' [host-scheduler]' || '' }}"
