...
export interface ProvisionOptions {
  /**
   * How to write the workflow file. The default (`undefined` or `true`) proposes
   * it via the `fixowl/provision-workflow` PR so a change to what runs on the
   * runner is reviewed before it lands. `false` (the `--no-pr` opt-out) pushes it
   * straight to the default branch. `--pr` is still accepted; it just names the
   * default.
   */
  pr?: boolean;
  noSchedule?: boolean;
...
export async function provisionCommand(
  ...
  for (const repoFullName of targetRepos(ctx.config, repoArg)) {
    ...
    // starter files below - it is proposed via PR by default. `--no-pr` opts out
    // and pushes it straight to the default branch.
    if (options.pr !== false) {
