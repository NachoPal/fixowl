...
| `fixowl provision [repo]` | Create labels, seal secrets, propose the fixowl workflow via PR, and register the runner on this host (spends the admin token's Administration: write). The workflow is always proposed on the `fixowl/provision-workflow` branch, never committed to the default branch - merge that PR to activate scheduled runs; `--no-schedule` generates a `workflow_dispatch`-only workflow (no cron); `--no-register` skips registration (register on the runner host with `start --register`). `--pr` is a deprecated no-op kept for one release. |
...
> `fixowl provision` proposes this workflow to your repos for you (always via a
> review PR on the `fixowl/provision-workflow` branch, never committed to the
> default branch), pinned to a released tag - you don't have to author it by hand.
