...
# - there are no hand-made `for: ci-e2e-triage` fixtures to maintain. Layer A skips before
# any agent runs, so this is a FREE scenario (zero LLM spend), validated by the free suite.
#
...
scenario_seed() {
  ...
  TA_FIXED="$(e2e_create_issue "[$RUN_TAG] already fixed by a merged PR" "$edit_body" "for: ci-e2e")"
  ...
  # The merge auto-closed the issue; reopen it so fixowl's OPEN-issue selection still sees it,
  # while the merged closing-keyword reference (the Layer-A already-fixed signal) persists.
  gh issue reopen -R "$R" "$TA_FIXED" >/dev/null

  # --- Fixture 2: duplicate (carries the `duplicate` label) --------------------------------
  TA_DUP="$(e2e_create_issue "[$RUN_TAG] duplicate issue" "$edit_body" "for: ci-e2e" "duplicate")"

  e2e_wait_visible "for: ci-e2e" "$TA_FIXED" "$TA_DUP" || return 1
...
scenario_cleanup() {
  ...
  e2e_cleanup_tracked "$TA_FIX_BRANCH"
}
