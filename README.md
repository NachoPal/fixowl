...
![fixowl architecture across four locations — your machine, GitHub, the runner host, and a per-issue Docker container — in ten numbered steps: one-time CLI setup (create the App, provision the repo, start the runner), then the nightly run (dispatch the job, select overnight issues, run the agent in a container, open one draft PR, and a CI-gated loop that flips the draft to ready when the required checks pass), and your morning review. fixowl never merges.](assets/how-it-works.svg)
